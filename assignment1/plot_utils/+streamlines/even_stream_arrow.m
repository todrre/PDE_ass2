...
if all(optional)
    xx = xx_or_xy;
    xy = streamlines.even_stream_data(xx, yy, uu, vv, min_density, max_density);
elseif ~any(optional)
    xy = xx_or_xy;
else
...
segment_length = sqrt(sum(diff(xy).^2, 2));
segment_length(isnan(segment_length)) = [];
mean_segment_length = mean(segment_length);
arrow_space = min(max(xy) - min(xy))/10/arrow_density; 
...
ha = streamlines.arrow(xy_from, xy_to, ...
    'Length', arrow_length, ...
